...
class TestPasswordRecovery:
    ...
    @pytest.mark.parametrize(
        'driver,email',
        [
            ['driver_chrome', helper.generate_payloads()],
            ['driver_chrome', helper.generate_payloads()],
            ['driver_firefox', helper.generate_payloads()],
            ['driver_firefox', helper.generate_payloads()]
        ]
    )
    ...
    def test_field_is_active_after_click_is_true(self, request, driver, email=helper.generate_payloads()):
        driver = request.getfixturevalue(driver)
        main_page = MainPage(driver)
        profile_page = ProfilePage(driver)
        recovery_page = RecoveryPage(driver)
